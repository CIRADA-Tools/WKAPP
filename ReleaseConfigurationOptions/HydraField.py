"""
    This module contains the definitions needed to analyze the Hydra field sources.  It contains the routines:
...
def FolderAndFileLoc(ParentFolder):
    ...
    SourceName="WALLABY Hydra DR2"
    #   Set the name of the folder where the analysis will be done
    DataReleaseFolder=ParentFolder+"HydraDR2Analysis/"
    #   Set the name of the folder containing the sources
    SourceFolder=DataReleaseFolder+"WALLABY Hydra DR2/"
    #   Set the base name of files in the release
    BaseName="WALLABY_PS_Hya_DR2"
    ...
    VelFolder=DataReleaseFolder+"Hydra_DR2_VelocityCubes_Jan2022/"
    ...
    MainBaroloFolder=DataReleaseFolder+"PythonBaroloAnalysis/"
    ...
    MainFATFolder=DataReleaseFolder+"FATAnalysis/"
    #   Name the csv file that will contain catalogue measurements
    MeasurementCatalogue=DataReleaseFolder+BaseName+"_CatalogueMeasurements_v1.csv"
...
def FATAnalysisOptions(FolderDict):
    ...
    FATAnalysisFolder=FolderDict['MainFATFolder']+"Feb2022_Analysis/"
    #   Name the catalogue file that FAT will need
    FATAnalysisCat=FATAnalysisFolder+FolderDict['BaseName']+"_FatCatalogue.txt"
...
def BaroloAnalysisOptions(FolderDict):
    ...
    SpecificAnalysisFolder="Feb_2022_Analysis/"
    #   Select whether to use smoothed or unsmoothed cubelets.
    UseSmoothedCubes=True
    #   Select whether to use SoFiA masks or the inbuilt masking software
    UseSoFiAMasks=False
    #   Set the number of Barolo fits for each galaxy
    nFitsPerGalaxy=1
...
def FitComparisonOptions(FolderDict):
    ...
    AvgModelReleaseBaseName="Wallaby_Hydra_DR2_KinematicModels_v"+str(ModelVersion)
    ...
    for i in range(nFATFits):
        if i ==0:
            ...
            FATConfigFileName[i]="FAT_INPUT_Hydra_DR2.config"
    #   Add in the provenance key values
    SBID="10609,10269,10612,10626"
    SRCVER="SoFiA 2.1.1 (14-Oct-2020)"
    SRCTR="Hydra TR2"
    KINVER="WKAPP v1"
    KINTR="Hydra Kin TR2"
