"""
    This module contains the definitions needed to analyze the Norma field sources.  It contains the routines:
...
def FolderAndFileLoc(ParentFolder):
    ...
    SourceName="WALLABY Norma DR1"
    #   Set the name of the folder where the analysis will be done
    DataReleaseFolder=ParentFolder+"NormaDR1Analysis/"
    ...
    VelFolder=DataReleaseFolder+"Norma_Vel_Cubes_Jan2022/"
...
def BaroloAnalysisOptions(FolderDict):
    ...
    SpecificAnalysisFolder="Feb_2022_Full_Analysis/"
    #   Select whether to use smoothed or unsmoothed cubelets.
    UseSmoothedCubes=True
...
def FitComparisonOptions(FolderDict):
    ...
    AvgModelReleaseBaseName="Wallaby_Norma_DR1_KinematicModels_v"+str(ModelVersion)
        #   Set the average model release folder
    ...
    for i in range(nBaroloFits):
        if i ==0:
            ...
            BaroloAnalysisFolders[i]=FolderDict['MainBaroloFolder']+SpecificAnalysisFolder+BaroloFitNames[i]+"/"
    ...
    for i in range(nFATFits):
        if i ==0:
            ...
            FATConfigFileName[i]="FAT_Norma_12kms.config"
    #   Add in the provenance key values
    SBID="12193,12209"
    SRCVER="SoFiA 2.3.1 (16-Feb-2021)"
    SRCTR="Norma TR1"
    KINVER="WKAPP v1"
    KINTR="Norma Kin TR1"
